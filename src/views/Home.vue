						...
						<v-card flat>
							<v-layout row wrap>
								...
								<v-flex xs12 v-if="!loadingData && hasData">
									...
									<v-expansion-panel v-model="panel" expand popout focusable :key="$store.state.global.updateView" :disabled="disableCalender">
												...
												<v-list-tile v-for="(data,indx) in type.data" :key="indx" :class="(data.hasOwnProperty('delete') && data.delete == true) ? 'red--text strikethrough' : '' ">
													...
													<v-list-tile-action>
														<span>
															...
															<v-btn small icon class="ma-2" @click="rowUpdate = data">
